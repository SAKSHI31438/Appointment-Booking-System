...
const ListOfAdmin = () => {
  ...
  const fetchAdmins = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/serviceProviderAdmin/getAllAdmins`,
      );
  ...
  const toggleStatus = async (id) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/serviceProviderAdmin/toggle-status/${id}`,
      );
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      {/* Filters */}
      <div className="flex flex-col md:flex-row gap-4 mb-6 w-full relative z-10">
        <input
          type="text"
          placeholder="Search by name or city..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="input-soft w-full md:w-1/2 lg:w-2/3"
        />
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-4 ">
                      <div className="rounded-full bg-yellow-100 p-1.5 cursor-pointer hover:bg-yellow-200 transition">
                        <PencilLine
                          onClick={() =>
                            navigate(`/admin/admins/edit/${p._id}`)
                          }
                          className="w-4 h-4 "
                        />
                      </div>
                      <div
                        onClick={() => navigate(`/admin/admins/view/${p._id}`)}
