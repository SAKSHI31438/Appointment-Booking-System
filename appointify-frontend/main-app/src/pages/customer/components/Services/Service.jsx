...
const Service = () => {
  ...
  const filteredProviders = providers.filter((provider) => {
    ...
    return matchesSearch && matchCategory && isActive;
  });
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/category/getAllActiveCategories`,
        );
    ...
    const fetchProviders = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/serviceProvider/getAllProviders`,
        );
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 mt-16">
      ...
      {/* Provider List */}
      <div className="divide-y divide-black bg-[#FFD3D5] rounded-xl">
        {loading ? (
          ...
          filteredProviders.map((provider) => (
            ...
            >
              ...
              <div className="w-full md:w-60 flex flex-col gap-4 border border-black rounded-xl p-4">
                <button
                  onClick={() => {
                    handleBookAppointment(provider);
                  }}
                  className="bg-[#540863] text-white py-2 rounded-lg hover:bg-[#390644] transition cursor-pointer"
                >
                  Book Appointment
                </button>

                <Link
                  to={`/provider-details/${provider._id}`}
