    ...
    <>
      ...
      >
        <nav className="max-w-7xl mx-auto px-4 sm:px-5 h-20 flex items-center justify-between">
          ...
          {isLoggedIn ? (
            <div className="flex gap-5  items-center cursor-pointer text-md font-medium">
              <NavLink
                to={"/myBookings"}
                className={({ isActive }) =>
                  `pb-1  px-2 transition hidden md:inline-flex  ${
                    isActive
                      ? "border-b-2 border-[#540863] text-white"
                      : "hover:text-[#540863] text-white"
                  }`
                }
              >
                My Bookings
      ...
      {nav && (
        <div
          className={`lg:hidden flex ${navOpen} text-white absolute  bg-[#92487A] max-w-full h-auto left-0 right-0 top-20 transform transition-all duration-500 delay-300  z-[1000] ease-in `}
        >
          {" "}
          <div className="flex flex-col w-full items-center cursor-pointer">
            ...
            {isLoggedIn ? (
              <div className="flex flex-col  w-full items-center cursor-pointer">
                ...
                <Link
                  onClick={handleLogout}
                  className="flex md:inline-flex p-1 w-full py-3 text-center items-center justify-center"
                >
                  Logout
                </Link>
              </div>
